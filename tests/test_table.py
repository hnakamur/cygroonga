            ...
            with grn.Groonga():
                with grn.Context() as ctx:
                    ...
                    table1 = ctx.create_table(table1_name,
                            ...
                            path=table1_path)
                    self.assertEquals(table1.name(), table1_name)
                    self.assertEquals(table1.path(), table1_path)
                    self.assertTrue(os.path.isfile(table1_path))
        finally:
        ...
        try:
            with grn.Groonga():
                with grn.Context() as ctx:
                    ...
                    table1.remove()
                    self.assertFalse(os.path.isfile(table1_path))
        finally:
            shutil.rmtree(work_dir)

# I got a "Segmentation fault: 11" error
#    def test_remove_closed_table(self):
#        work_dir = tempfile.mkdtemp()
#        try:
#            with grn.Groonga():
#                with grn.Context() as ctx:
